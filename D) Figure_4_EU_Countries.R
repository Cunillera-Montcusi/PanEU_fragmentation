...
# We charge TOTAL freshwater Data. We only need one because later we charge the already treated dataset
source("functions/SLURM_outputs_treat_Total.R")

# And then charge the following
load("outputs/Final_treated_data.RData")
...
Perc_Countries <- read.csv2("data/Percentage_per_Countries.csv",dec = ".") %>%
