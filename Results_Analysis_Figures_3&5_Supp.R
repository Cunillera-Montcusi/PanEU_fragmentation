...
EU_relat_TOTAL[[1]] <- EU_relat_TOTAL[[1]]%>%left_join(read.csv2("NoComp_ECOR.csv"),by=c("EcoR"="Number_EcoR"),relationship = "many-to-many") %>%
...
corrplot::corrplot(cor(data_to_gam_scaled, use="complete.obs", method="spearman"),
                   method="ellipse", tl.col = "black", tl.cex = 0.5, tl.srt = 60, type=c("lower"), cl.length=9)
...
EU_relat_Perm[[1]] <- EU_relat_Perm[[1]]%>%left_join(read.csv2("NoComp_ECOR.csv"),by=c("EcoR"="Number_EcoR"),relationship = "many-to-many") %>%
                     filter(Name!="Unassigned") %>%  filter(X!="no complet") %>% 
...
EU_relat_Tempo[[1]] <- EU_relat_Tempo[[1]]%>%left_join(read.csv2("NoComp_ECOR.csv"),by=c("EcoR"="Number_EcoR"),relationship = "many-to-many") %>%
  filter(Name!="Unassigned") %>%  filter(X!="no complet") %>% 
...
EU_relat_Ephem[[1]] <- EU_relat_Ephem[[1]]%>%left_join(read.csv2("NoComp_ECOR.csv"),by=c("EcoR"="Number_EcoR"),relationship = "many-to-many") %>%
  filter(Name!="Unassigned") %>%  filter(X!="no complet") %>% 
