...
png(filename ="Loss_Alph_Bet_Perm.png",
...
for (ID_Ecoregion in 1:length(Outputs_SLURM)){
  ...
  for (dispersal in 1:length(dispersal_distances)) {
    ...
    model_S <- nls(s~(a-b*f)-exp(f^q),start=list(a=s[1],b=0.1, q=1), control = list(maxiter=1000000))
...
png(filename ="Loss_Gamm_Perm.png",
...
# Plot EU map with coefficients for permanent freshwaters ####
plot_amazing <- list()
...
png(filename ="EU_degr_Perm.png",width =350*7 ,height =800*7 ,units ="px",res = 300)
