...
EcoR_size_load <- EcoR_size %>% filter(EcoReg_ID!=406)%>% filter(EcoReg_ID!=404)
...
png(filename ="Loss_Alph_Bet.png",
...
for (ID_Ecoregion in 1:length(Outputs_SLURM)) {
  ...
  for (dispersal in 1:length(dispersal_distances)) {
    ...
    model_S <- nls(s~(a-b*f)-exp((f^q)/(fo^q)),start=list(a=s[1],b=0.0001, fo=11, q=1.3), control = list(maxiter=10000))
    
    p<-coefficients(model_S)
    out_coef_b[dispersal] <- p[2]
    out_coef_f0[dispersal] <- p[3]
